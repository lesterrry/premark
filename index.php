	...
	if ($data == null) {
		...
		if (!isset($data['title'])) {
			...
			if (!isset($data['totalMark']) || is_null($data['totalMark'])) {
				q($title . "\nНу и ну! Администрация портфолио, по всей видимости, сочла этот проект вредоносным, и дала мне дружескую рекомендацию его ликвидировать. Что-ж, это было чудесное приключение! 4318 проектов, 1432 постоянных пользователя и целых 17.428 запросов! Потрясающе. Я искренне надеюсь и считаю, что разрабатывая премарк, старался привнести в временами суровую шдшную жизнь больше спокойствия, прозрачности и взаимодействия. И поверьте, я продолжу этот путь, но уже в других проектах. Если вы это читаете — спасибо, что были здесь! Предсказуемого и удачного остатка сессии, не теряйтесь, и, конечно,\n❇️ Подписывайтесь на канал: @cockhunter\n\n_Farewell!_");
			} else {
				$mark = $data['totalMark'];
...
if (TEXT == '/start') {
	q('Привет! Я помогал узнать предварительную оценку проекта. Отправь ссылку на проект в формате https://portfolio.hse.ru/Project/159642');
} else if (TEXT == '/poll') {
	qPoll(POLL[0], POLL[1]);
